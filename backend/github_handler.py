...
def download_github_repo(repo_url: str) -> str:
    ...
    # 3. Build headers — User-Agent is required by GitHub API, otherwise it can return 403
    headers = {
        "User-Agent": "ai-repo-analyzer-app",
        "Accept": "application/vnd.github+json"
    }
    
    # If a GITHUB_TOKEN is set in environment variables, use it for authentication
    # This raises the rate limit from 60/hour to 5000/hour
    github_token = os.environ.get("GITHUB_TOKEN")
    if github_token:
        headers["Authorization"] = f"Bearer {github_token}"
    
    # 4. Make the network request to download the file
    response = requests.get(api_url, headers=headers)
    
    if response.status_code != 200:
        raise Exception(f"Failed to download from GitHub. Make sure the repository is public. Status: {response.status_code}")
        
    # 5. Create a safe, temporary file path on your computer to store the .zip
    temp_dir = tempfile.gettempdir()
    zip_file_path = os.path.join(temp_dir, f"{repo}_codebase.zip")
    
    # 6. Save the downloaded data into the .zip file
    with open(zip_file_path, "wb") as file:
        file.write(response.content)
